...
final class V1PlayerLimbs implements BmPlayerLimbs {

    private final V1Platform platform;
    private final Plugin ownPlugin;

    V1PlayerLimbs(V1Platform platform, Plugin ownPlugin) {
    ...
    public boolean playLimbAnimation(Player player, String limbModel, String animation, BmAnimationOptions options) {
        boolean started = BetterModel.plugin().modelManager().animate(player, limbModel, animation,
                V1Tracker.modifierOf(options).build());
        if (started) {
            ...
            BetterModel.registry(player.getUniqueId())
                    ...
                    .ifPresent(tracker -> platform.markStarted(tracker, animation,
                            V1Tracker.typeOf(options.loopMode()).name()));
        }
        return started;
    }
    ...
    public CompletableFuture<SkinPartResult> applySkinPart(Supplier<Optional<Entity>> target,
        ...
        ModelProfile.Uncompleted sourceProfile = ModelProfile.of(source).asUncompleted();
        ...
        skinManager.complete(sourceProfile).thenAccept(skinData ->
                schedule(known, () -> future.complete(SkinPartResult.TARGET_GONE), () -> {
                    try {
                        ...
                        EntityTracker tracker = BetterModel.registry(entity)
                        ...
                        var adaptedSource = BetterModel.plugin().nms().adapt(source);
