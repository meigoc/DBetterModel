...
final class V3Bone implements BmBone {

    private final Tracker tracker;
    private final RenderedBone bone;
    private final V3BoneState state;

    V3Bone(Tracker tracker, RenderedBone bone, V3BoneState state) {
    ...
    public Quaternionf hitboxViewRotation() {
        // RenderedBone.hitBoxViewRotation() was dropped in 3.x; same math as the 1.x impl.
        var rotation = bone.rotation();
        return MathUtil.toQuaternion(bone.worldRotation())
    ...
    public Location worldLocation() {
        PlatformLocation base = tracker.location();
        if (!(base instanceof BukkitLocation bukkitLocation)) return null;
        Vector3f pos = bone.worldPosition();
        return new Location(bukkitLocation.source().getWorld(), pos.x(), pos.y(), pos.z());
    }
    ...
    public void setVisible(boolean visible, List<Player> players) {
        ...
        PacketBundler bundler = tracker.getPipeline().createBundler();
        if (visible) {
    ...
    public void setViewRange(float range) {
        ...
        if (display != null) {
            ...
            // 3.x viewedPlayer() yields PlayerChannelHandler, not players (matrix §9).
            tracker.getPipeline().viewedPlayer().forEach(handler -> bundler.send(handler.player()));
        }
    }
    ...
    public boolean setBillboard(Display.Billboard billboard) {
        ...
        PlatformBillboard platformBillboard = PlatformBillboard.valueOf(billboard.name());
        if (bone.applyAtDisplay(BonePredicate.TRUE, display -> display.billboard(platformBillboard))) {
            forceUpdate();
            return true;
        }
        return false;
